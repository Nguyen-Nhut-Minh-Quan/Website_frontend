...
async function fetchtanktext(tankId) {
    try {
        console.log(`${API_BASE_URL}/text/${Tank_Location}/${tankId}`);
        const baseUrl = `${API_BASE_URL}/text/${Tank_Location}/${tankId}`;
...
async function UpdateTemperatureChart(tankId) {
    ...
    const TempData = await fetchtanktemp(tankId);
    if (TempData.length <= 0) {
        Chart.innerHTML = '<div class="alert alert-info text-center" role="alert" style = "height:275px">Loading Chart Data...</div>';
        return;
    }
    let Layer1temp = [];
    let Layer2temp = [];
    ...
    const commonOptions = {
        ...
        xaxis: {
            ...
            type: 'category',
            labels: {
                style: { show: false } // White labels for dark background
            },
        },
        yaxis: {
            axisBorder: {
              ...
              <div class="card-body" >
                <ul class="list-group list-group-flush">
                  <li class="list-group-item d-flex justify-content-between align-items-center">
                    Date: <span class="badge bg-label-info" id = "${tankId}_Date_temperature_text">N/A </span>
                  </li>
                  <li class="list-group-item d-flex justify-content-between align-items-center">
                    Time: <span class="badge bg-label-info" id = "${tankId}_Time_temperature_text">N/A </span>
                  </li>
                  <li class="list-group-item d-flex justify-content-between align-items-center">
...
async function UpdateTankTemperatureText(tankId) {
    const DateSpan = document.getElementById(`${tankId}_Date_temperature_text`);
    const TimeSpan = document.getElementById(`${tankId}_Time_temperature_text`);
    ...
    const FullTime = info['Timestamp'];
    const dateUST = new Date(FullTime);
...
async function populateServersInTankCards(tank) {
    ...
    for (const server of CardValues) {
        ...
        const ram_total = await formatSize(server.total_ram_mb);
        const ram_used = await formatSize(server.used_ram_mb);
        ...
        <div class="col-xl-2 col-lg-3 col-md-4 col-sm-6 mb-3">
          <div class="card physical-server-card" data-server-id="${server.SERVER_ID}" >
            <div class="card-header py-1 px-2">
                <h6 class="mb-0 text-truncate">Server ${server.SERVER_IP}</h6>
            </div>
            <div class="card-body">
